...
if [[ "$serverurl" =~ ^((udp|tcp)(4|6)?://)?([0-9a-zA-Z\.\-]+)(:([0-9]+))?$ ]]; then
    ...
    echo "eg: tcp://my.fully.qualified.domain.com:443"
    exit 1
fi
...
kuberes='./kube-resources'
mkdir -p $kuberes
kubedir='./kube-openvpn'
...
kubectl apply --namespace=$namespace -f $kubedir/configmaps-example.yaml
...
kubectl create configmap --namespace=$namespace openvpn-template --from-file=$kubedir/openvpn.tmpl
sed "s/\${OVPN_CN}/${OVPN_CN}/g;" $kubedir/deployment.yaml | kubectl create --namespace=$namespace -f -
